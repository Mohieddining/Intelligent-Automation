...
def test_make_me_a_graph():
    ...
    This will however pass if you return anything, so it may not be correct. That you
    will only know when all tests pass in this file. Use the debugger to check your
...
def test_source_vertices():
    """
    test the source_vertices method so the source is returned
    """
    (g,vs, es, s0, s1, s2, s3, e1, e2, e3, e4, e5) = setup()

    assert g.source_vertices() == [s0]
